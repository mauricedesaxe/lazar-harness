...
5. **Publish the spec and obtain product approval.** Call **matt-to-spec** after shared understanding or a clear Wayfinder map. Publish the unapproved draft on the resolved tracker as `ready-for-human`, or leave it unlabeled. Never apply `ready-for-agent` to an unapproved draft. A GitHub-backed repository gets a GitHub spec issue. Obtain the human's approval for the final published body. Then record it on the spec as a tracker comment in the form `Approved-Spec-Body-SHA256: <hash>`. Hash the tracker's current body, not a local draft. Use a native immutable body revision when the tracker has one. Otherwise hash a deterministic body export. For GitHub, use `gh issue view <spec-number> --json body | jq -cS . | sha256sum | cut -d ' ' -f1`. Use `shasum -a 256` in place of `sha256sum` when needed. Post the marker only after the human approves the body that produced the hash. Only after the marker exists may Product shaping replace `ready-for-human` with `ready-for-agent`. Do not start implementation before the marker exists.
<!-- surface:local -->
